...
export interface PolicyRule {
	...
	/** NetBird user group ID -> permitted local OS usernames for NetBird SSH. */
	authorizedGroups?: Readonly<Record<string, ReadonlyArray<string>>>;
...
/** Dashboard-managed default policy — re-enabled, never deleted, on stack destroy. */
...
type ApiRule = {
	...
	authorized_groups?: Readonly<Record<string, ReadonlyArray<string>>>;
...
export const PolicyProvider = () =>
	Provider.succeed(Policy, {
		...
		reconcile: Effect.fn(function* ({ id, news, output }) {
			...
			if (props.rules !== undefined && props.rules.length > 1) {
				return yield* Effect.die(
					new Error(
						`NetBird.Policy "${name}" has ${props.rules.length} rules — NetBird 0.75 only persists one rule per policy; use one rule per Policy resource`,
					),
				);
			}
		...
		delete: Effect.fn(function* ({ output }) {
			if (output.name === DEFAULT_POLICY_NAME) {
				yield* restoreDefaultPolicy(output.policyId);
				return;
			}
...
/**
 * The dashboard Default policy is adopted, never owned. Destroying the
 * resource re-enables it so a stack that disabled Default cannot leave the
 * mesh without a single allow rule.
 */
const restoreDefaultPolicy = (policyId: string) =>
	Effect.gen(function* () {
		const current = yield* catchNotFoundOrUnavailable(policiesPolicyIdGet({ policyId }));
		if (!current || current.enabled) return;
		const policy = normalizePolicy(current);
		yield* policiesPolicyIdPut({
			policyId,
			name: policy.name,
			...(policy.description !== undefined ? { description: policy.description } : {}),
			enabled: true,
			source_posture_checks: policy.source_posture_checks,
			rules: observedRulesToApi(policy.rules),
		});
	});
...
const toApiRule = (rule: PolicyRule, id?: string) => ({
	...
	...(rule.portRanges !== undefined
		...
		: {}),
	...(rule.authorizedGroups !== undefined ? { authorized_groups: rule.authorizedGroups } : {}),
		...
		toApiRule(
			{
				...
				...(rule.authorized_groups !== undefined ? { authorizedGroups: rule.authorized_groups } : {}),
...
const normalizeRule = (rule: {
	...
	JSON.stringify({
		...
		authorized_groups: Object.fromEntries(
			Object.entries(rule.authorized_groups ?? {})
				.sort(([a], [b]) => a.localeCompare(b))
				.map(([group, users]) => [group, [...users].sort()]),
		),
