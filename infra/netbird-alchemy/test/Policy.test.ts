...
test.provider.skipIf(!isDockerReady)("disable and restore the dashboard Default policy without deleting it", stack =>
	Effect.gen(function* () {
		yield* fixture;
		...
		const disabled = yield* policiesPolicyIdGet({ policyId: policy.policyId });
		...
		expect(afterDestroy.enabled).toBe(true);
	}).pipe(withLogLevel),
);
