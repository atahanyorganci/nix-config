...
install_setup_key() {
    local setup_key=$1
    local key_dir remote_cmd
    key_dir=$(dirname "$SETUP_KEY_FILE")

    # Expand constant paths on the client; printf %q escapes for the remote shell.
    if [[ $OS == nixos ]]; then
        remote_cmd="/run/wrappers/bin/sudo mkdir -p $(printf %q "$key_dir") && /run/wrappers/bin/sudo tee $(printf %q "$SETUP_KEY_FILE") > /dev/null && /run/wrappers/bin/sudo chmod 600 $(printf %q "$SETUP_KEY_FILE")"
    else
        remote_cmd="sudo mkdir -p $(printf %q "$key_dir") && sudo tee $(printf %q "$SETUP_KEY_FILE") > /dev/null && sudo chmod 600 $(printf %q "$SETUP_KEY_FILE")"
    fi

    printf '%s\n' "$setup_key" | ssh "${SSH_OPTS[@]}" "${SSH_USER}@${SSH_TARGET}" "$remote_cmd"
}
