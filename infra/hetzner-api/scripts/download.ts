...
const { url: specUrl, sha256 } = pkg.spec;
...
/** `action_id` / `peer-id` → `actionId` / `peerId` for Schema.Struct keys */
const toJsIdent = (name: string): string => {
	const camel = name.replace(/[_-]([a-zA-Z0-9])/g, (_, c: string) => c.toUpperCase()).replace(/[^a-zA-Z0-9_$]/g, "");
	if (!camel) return name;
...
/** Rename snake_case / hyphenated path params so the OpenAPI generator emits camelCase TS. */
...
/** Always derive noun-verb ids from method+path (ignore upstream verb_noun ids). */
...
const program = Effect.gen(function* () {
	...
	const outPath = path.join(scriptDir, "..", "spec", "hetzner.cloud.api.json");

	const response = yield* http.get(specUrl).pipe(Effect.flatMap(HttpClientResponse.filterStatusOk));
	const raw = yield* response.text;

	const digest = yield* crypto.digest("SHA-256", new TextEncoder().encode(raw));
	const hash = toHex(digest);

	if (hash !== sha256) {
		return yield* Effect.fail(new Error(`Spec content hash mismatch: expected ${sha256}, got ${hash}`));
	}

	const doc = yield* Effect.sync(() => JSON.parse(raw) as OpenApiDoc);
	...
	yield* Console.log(
		`Wrote ${outPath} (sha256:${hash}, ${assigned} operationIds, ${deduped} deduped, ${renamedParams} path params renamed)`,
	);
});
